...
# NEU Metal Surface Defects classes
DEFAULT_CLASS_NAMES = [
    "crazing",
    "inclusion",
    "patches",
...
# This avoids the heavy Ultralytics + PyTorch dependency.
...
def get_class_names():
    """
    Try to read class names stored inside the ONNX model metadata.
    If metadata is unavailable, use the NEU defect class names.
    """
        ...
        if names:
            try:
                ...
                if isinstance(parsed_names, dict):
                    return [
                        parsed_names[str(index)]
                        if str(index) in parsed_names
                        else parsed_names.get(index, str(index))
...
def preprocess_image(image):
    """
    Prepare OpenCV BGR image for YOLO ONNX inference.
    """

    processed_image, scale, pad_x, pad_y = (
...
def postprocess_predictions(
    ...
    """
    Convert YOLO ONNX output into bounding boxes,
    ...
    # Standard YOLOv8 ONNX output is usually:
    # [1, 4 + number_of_classes, number_of_predictions]
    #
    # Convert it to:
    # [number_of_predictions, 4 + number_of_classes]
